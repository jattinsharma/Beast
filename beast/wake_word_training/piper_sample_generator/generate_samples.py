...
def generate_samples(text, max_samples, batch_size, noise_scales, noise_scale_ws, length_scales,
    ...
    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'piper_voices', 'en_US-lessac-medium.onnx')
        ...
        try:
            ...
            for i, txt in enumerate(batch_texts):
                ...
                syn_config = SynthesisConfig(
                    ...
                    noise_w_scale=noise_scale_ws[0] if isinstance(noise_scale_ws, list) else noise_scale_ws
                )
                audio_gen = voice.synthesize(txt, syn_config=syn_config)

                # Convert generator of AudioChunks to numpy array
                audio_chunks = list(audio_gen)
                audio_arrays = [chunk.audio_float_array for chunk in audio_chunks]
                audio = np.concatenate(audio_arrays)

                # Piper outputs at 22050 Hz, resample to 16000 Hz for consistency
                original_sample_rate = audio_chunks[0].sample_rate  # All chunks have the same sample rate
                ...
                # Save as WAV file at the target sample rate (16000 Hz)
                wavfile.write(filepath, target_sample_rate, audio_int16)
